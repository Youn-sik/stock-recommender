...
class DaumFinanceCrawler:
    """다음 금융 뉴스 크롤러"""
    
    def __init__(self):
        self.base_url = "https://finance.daum.net"
        self.news_url = "https://finance.daum.net/news"
        self.headers = {
    ...
    async def crawl_articles(self, keywords: List[str] = None, max_articles: int = 50) -> List[Dict]:
        """다음 금융 뉴스 크롤링"""
        logger.info(f"Starting Daum Finance crawling (max: {max_articles} articles)")
        ...
        try:
            ...
            logger.error(f"Error crawling Daum Finance: {str(e)}")
        
        # 중복 제거
        unique_articles = self._remove_duplicates(articles)
        logger.info(f"Collected {len(unique_articles)} unique articles from Daum Finance")
    ...
    async def _crawl_main_news(self, session: aiohttp.ClientSession, max_articles: int) -> List[Dict]:
        ...
        try:
            async with session.get(self.news_url) as response:
                ...
                # 뉴스 링크 추출 (다음의 HTML 구조에 맞게 조정)
                news_links = soup.select('.list_news .link_txt')
    ...
    async def _crawl_search_news(self, session: aiohttp.ClientSession, keyword: str, max_articles: int) -> List[Dict]:
        ...
        try:
            search_url = f"https://search.daum.net/search?w=news&q={keyword} 주식"
            ...
            async with session.get(search_url) as response:
                ...
                # 검색 결과에서 뉴스 링크 추출
                news_items = soup.select('.c-list-basic .item-title .link-txt')
                
                # 각 뉴스 기사 상세 정보 수집
                tasks = []
                for link in news_items[:max_articles]:
                    article_url = link.get('href')
                    
                    # 다음 뉴스 링크만 처리
                    if article_url and 'news.daum.net' in article_url:
                        tasks.append(self._crawl_article_detail(session, article_url))
    ...
    async def _crawl_article_detail(self, session: aiohttp.ClientSession, url: str) -> Dict:
        ...
        try:
            async with session.get(url) as response:
                ...
                # 제목 추출
                title_elem = soup.select_one('.tit_view') or soup.select_one('h3.tit_view')
                title = title_elem.get_text().strip() if title_elem else ""
                
                # 내용 추출
                content_elem = soup.select_one('.article_view') or soup.select_one('div.article_view')
                content = ""
                if content_elem:
                    # 불필요한 태그 제거
                    for script in content_elem(["script", "style", "figure"]):
                        script.decompose()
                    content = content_elem.get_text().strip()
                
                # 발행일 추출
                date_elem = soup.select_one('.num_date') or soup.select_one('span.num_date')
    ...
    def _parse_date(self, date_text: str) -> datetime:
        ...
        try:
            # "2024.07.13. 오후 3:30" 형식 파싱
            date_pattern = r'(\d{4})\.(\d{2})\.(\d{2})\.\s*(오전|오후)\s*(\d{1,2}):(\d{2})'
            ...
            match = re.search(month_day_pattern, date_text)
            if match:
